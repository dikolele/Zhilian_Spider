...
public class MultiplexingHttpConectionPool {
	...
	public void initialize(int cmrConnMaxTotal , int cmrDefaultMaxPerRoute ,
		...
		sockConf = 
				SocketConfig.custom().setSoReuseAddress(true)
				.setTcpNoDelay(true).setSoLinger(0).build();
		reqConf = 
				RequestConfig.custom().setConnectionRequestTimeout(connRequestTimeout)
				.setConnectTimeout(connTimeout).setSocketTimeout(socketTimeout).build();
		clientBuilder = 
				HttpClients.custom().setDefaultSocketConfig(sockConf).setDefaultRequestConfig(reqConf)
				.setConnectionManager(cmr).setRetryHandler(new RetryHandler());
	}
